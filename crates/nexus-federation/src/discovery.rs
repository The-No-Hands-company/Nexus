...
impl DiscoveryCache {
    ...
    ///
    /// # Errors
    /// Returns an error if discovery fails for all supported strategies.
    ...
    async fn do_resolve(&self, server_name: &str) -> Result<String, FederationError> {
        ...
        if has_explicit_port(server_name) {
            ...
            if self.probe_base_url(&https_base).await {
                debug!(
                    "Discovery (explicit port, HTTPS): {} → {}",
                    server_name, https_base
                );
                return Ok(https_base);
            }
            let explicit_http_base = format!("http://{server_name}");
            debug!(
                "Discovery (explicit port, HTTP fallback): {} → {}",
                server_name, explicit_http_base
            );
            return Ok(explicit_http_base);
        }
        ...
        if is_local_address(server_name) {
            let base = format!("http://{server_name}:8080");
    ...
    async fn probe_base_url(&self, base_url: &str) -> bool {
        ...
        match self.http.get(&url).send().await {
            ...
            Ok(_) | Err(_) => true, // other errors mean server responded somehow
        }
    }
    ...
    async fn try_well_known(&self, server_name: &str) -> Option<String> {
        ...
        for scheme in &["https", "http"] {
            let url = format!("{scheme}://{server_name}/.well-known/nexus/server");
            if let Ok(resp) = self.http.get(&url).send().await
                && resp.status().is_success()
                && let Ok(wk) = resp.json::<WellKnownServer>().await
            {
                // ── SSRF guard ────────────────────────────────────────
