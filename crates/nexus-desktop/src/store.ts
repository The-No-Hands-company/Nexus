...
class Store {
  ...
  async loadMessages(channelId: string, before?: string) {
    try {
      ...
      if (before) {
        ...
        this.messages = { ...this.messages, [channelId]: sortMessages([...existing, ...msgs]) };
      } else {
        this.messages = { ...this.messages, [channelId]: sortMessages(msgs) };
      }
      this.emit();
  ...
  async sendMessage(content: string, replyTo?: string) {
    ...
    this.messages = {
      ...this.messages,
      [this.activeChannelId]: sortMessages([...(this.messages[this.activeChannelId] ?? []), optimistic]),
    };
    this.emit();
    try {
      ...
      this.messages = {
        ...this.messages,
        [this.activeChannelId]: sortMessages((this.messages[this.activeChannelId] ?? []).map(m => m.id === optimistic.id ? sent : m)),
      };
      this.emit();
      return;
    } catch (e) {
  ...
  appendMessage(msg: Message) {
    ...
    const msgs = sortMessages([msg, ...existing]);
  ...
  private toggleReaction(channelId: string, messageId: string, emoji: string, add: boolean) {
    ...
    this.messages = {
      ...this.messages,
      [channelId]: sortMessages(msgs.map(m => {
        if (m.id !== messageId) return m;
        const reactions = [...(m.reactions ?? [])];
        const idx = reactions.findIndex(r => r.emoji === emoji);
        if (add) {
