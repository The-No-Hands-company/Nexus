...
use nexus_db::repository::{
    scaling_configs, sfu_nodes, federation_batches, voice_quality, large_server, ops_metrics, members,
};
...
pub fn router() -> Router<Arc<AppState>> {
    Router::new()
        ...
        // SFU nodes
        .route("/admin/sfu-nodes", get(list_sfu_nodes_handler).post(upsert_sfu_node_handler))
        // Federation batches
        .route("/admin/federation-batches", get(list_federation_batches_handler))
        ...
        // Ops metrics
        .route("/admin/scaling-metrics", get(list_metrics_handler).post(record_metric_handler))
        .route(
...
#[derive(Debug, Deserialize)]
struct RegionQuery { region: Option<String> }

#[derive(Debug, Deserialize)]
struct InstanceQuery { instance_id: Option<String> }
...
async fn upsert_scaling_config_handler(
    ...
    let row = scaling_configs::upsert_scaling_config(
        &state.db.pool, Uuid::new_v4(), &body.instance_id, &body.region,
        &body.shard_strategy.unwrap_or_else(|| "consistent_hash".into()),
...
async fn get_scaling_config_handler(
    ...
    let row = scaling_configs::get_scaling_config(&state.db.pool, config_id)
        .await.map_err(|e| NexusError::Internal(e.into()))?
        .ok_or(NexusError::NotFound { resource: "scaling_config".into() })?;
    Ok(Json(row))
}
...
async fn upsert_sfu_node_handler(
    ...
    let row = sfu_nodes::upsert_sfu_node(
        &state.db.pool, Uuid::new_v4(), &body.instance_id, &body.region,
        &body.hostname, body.port.unwrap_or(443),
...
async fn list_voice_quality_logs_handler(
    ...
    let rows = voice_quality::list_voice_quality_logs(&state.db.pool, channel_id, q.limit.unwrap_or(100))
        .await.map_err(|e| NexusError::Internal(e.into()))?;
    Ok(Json(rows))
}
...
async fn create_voice_quality_log_handler(
    ...
    let row = voice_quality::create_voice_quality_log(
        &state.db.pool, Uuid::new_v4(), channel_id, body.user_id,
        body.sfu_node_id, body.bitrate.unwrap_or(64000),
        body.packet_loss.unwrap_or(0.0), body.jitter_ms.unwrap_or(0.0),
        body.latency_ms.unwrap_or(0), body.fec_enabled.unwrap_or(false),
...
async fn upsert_prune_rule_handler(
    ...
    let row = large_server::upsert_prune_rule(
        &state.db.pool, Uuid::new_v4(), server_id,
...
async fn upsert_slow_mode_override_handler(
    ...
    let row = large_server::upsert_slow_mode_override(
        &state.db.pool, Uuid::new_v4(), channel_id, body.role_id,
        body.cooldown_secs, body.escalation_mult.unwrap_or(1.0),
    ).await.map_err(|e| NexusError::Internal(e.into()))?;
    Ok(Json(row))
}
...
async fn record_metric_handler(
    ...
    let row = ops_metrics::record_metric(
        &state.db.pool, Uuid::new_v4(), &body.instance_id, &body.metric_name,
        body.metric_value,
...
async fn create_upgrade_record_handler(
    ...
    let row = ops_metrics::create_upgrade_record(
        &state.db.pool, Uuid::new_v4(), &body.from_version, &body.to_version,
        body.notes.as_deref(),
        &body.metadata.unwrap_or(serde_json::json!({})),
    ).await.map_err(|e| NexusError::Internal(e.into()))?;
    Ok(Json(row))
